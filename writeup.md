...
My pipeline consisted of 6 steps. The pipeline is shown below with output for each step:
...
* Then I applied Gaussian Blur to reduce noise in the image so that Canny edge detection can pick up relevant edges
...
* A mask was designed keeping in mind the perspective view of the lane lines, this helps to focus on region of interest when perform line detection using Hough transform
...
* The objective of the project was to mark the lane markers with a continuous lines. The figure below shows the slope and intercept of each line detected by Hough function (calculated using [x1, y1] and [x2, y2]). It is observed that the point cloud can be easily separated into two classes.
<img src="./pipeline_steps/step_7_Slope_intercept_KMean.png" width="480" alt="Combined Image"/>
I employed k-mean clustering to cluster the point cloud (circular dots) in two regions and calculated the mean (plotted using squares with a cross) slope and intercept for two clusters. This provides me with an average slope of left and a right lane marker.
<img src="./pipeline_steps/solidYellowCurve.jpg" width="480" alt="Combined Image"/>

* While processing the "Challenge" video it was noticed that a lot of horizontal lines were detected. I tried tuning different parameters in the pipeline, but there were still many difficulties. Hence, I defined an ignore zone in the slope-intercept plot for lane line detection. As lane markers can never be horizontal, I ignored the lines which have slopes between -0.1 to +0.1.
...
* The pipeline will behave poorly if there is a car in the region of interest as the algorithm does not know how to distinguish between objects on road and lane markers.
